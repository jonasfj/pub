...
/// SDK VERSION set in pub when running tests, see [getPubTestEnvironment].
const _pubTestSdkVersion = '0.1.2+3';
...
Map<String, String> getPubTestEnvironment([String tokenEndpoint]) {
  var environment = {
    ...
    '_PUB_TEST_SDK_VERSION': _pubTestSdkVersion,
  };
...
/// Create `pubspec.lock` if [lockFile] is `true` and `.packages` if
/// [packagesFile] is `true`, without running `pub get`.
///
/// This assumes [package] is both the _name of the package_, and the directory
/// in the [d.sandbox] where `.packages` and `pubspec.lock` should be created.
///
/// The [hosted] argument is a mapping from package name to package version,
/// referencing packages in the `PUB_CACHE` installed from [globalServer].
///
/// The [sandbox] argument is a mapping from package name to package version,
/// referencing packages in [d.sandbox] stored at the package name relative
/// to [d.sandbox].
///
/// This will set SDK and language versions to [_pubTestSdkVersion].
Future<void> createLockAndPackageFile(
  String package, {
  Map<String, String> hosted = const {},
  Map<String, String> sandbox = const {},
  bool lockFile = true,
  bool packagesFile = true,
}) async {
  final cache = _pathInSandbox(cachePath);
  final dotPackages = StringBuffer();
  packages_file.write(
    dotPackages,
    {
      ...hosted.map((package, version) => MapEntry(
          package,
          Uri(
            scheme: 'file',
            path: p.join(
              cache,
              'hosted',
              'localhost%58${globalServer.port}/$package/$version/lib/',
            ),
            fragment: 'dart=$_pubTestSdkVersion',
          ))),
      ...sandbox.map((package, version) => MapEntry(
            package,
            Uri(
              path: '../$package/lib/',
              fragment: 'dart=$_pubTestSdkVersion',
      ...
      package: Uri(path: 'lib/', fragment: 'dart=$_pubTestSdkVersion'),
      '': Uri(path: package), // Default package name
    },
    comment: 'Generated by pub tests on ${DateTime.now()}.',
  ...
  await d.dir(package, [
    if (lockFile)
      d.file(
        'pubspec.lock',
        yamlToString({
          ...hosted.map((package, version) => MapEntry(package, {
                'dependency': 'direct main',
                'description': {
                  'name': package,
                  'url': 'http://localhost:${globalServer.port}',
                },
                'source': 'hosted',
                'version': version,
              })),
          ...sandbox.map((package, version) => MapEntry(package, {
                'dependency': 'direct main',
                'description': {
                  'path': '../$package',
                  'relative': true,
          ...
          'sdks': {
            'dart': '>=$_pubTestSdkVersion <=$_pubTestSdkVersion',
          },
        }),
      ),
    if (packagesFile) d.file('.packages', dotPackages.toString()),
  ]).create();
}
